...
class UserScreen(QWidget):
    ...
    def run_auth_flow(self):
        ...
        if auth_server.request_token:
            if self.session_manager.generate_session(auth_server.request_token):
                ...
                holdings = self.session_manager.get_kite().holdings()
                self.comm.update_status_signal.emit(
                    "Login successful", True, margins, positions, holdings
                )
            else:
                self.comm.update_status_signal.emit(
                    "Status: Login failed", False, None, None, None
                )
        else:
            self.comm.update_status_signal.emit(
                "Status: Login failed", False, None, None, None
    ...
    def update_status(
        self, message, success, margins=None, positions=None, holdings=None
    ):
        if success:
    ...
    def update_dashboard(self, margins, positions, holdings):
        # Funds
        if margins:
            self.funds_label.setText(
                f"Available: ₹{margins['equity']['available']['live_balance']:.2f}"
            )
        ...
        if positions and (positions["net"] or positions["day"]):
            ...
            positions_df = positions_df[
                ["tradingsymbol", "quantity", "average_price", "pnl", "m2m"]
            ]
