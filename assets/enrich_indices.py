...
def fetch_constituents(index_name, url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to fetch {index_name} (Status: {response.status_code})")
            return []
...
def main():
    ...
    try:
        main_df = pd.read_csv('assets/master_catalog.csv')
    ...
    output_file = 'assets/master_catalog_enriched.csv'
    main_df.to_csv(output_file, index=False)
    print(f"\nSuccess! Saved enriched data to {output_file}")
    print("Sample Row:")
    print(main_df[main_df['Symbol'] == 'RELIANCE']
          [['Symbol', 'Indices']].to_string())
